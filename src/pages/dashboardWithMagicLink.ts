...
export async function getLatestMagicLinkFromOutlook(browser: Browser, triggerTime: number): Promise<string> {
  ...
  while (Date.now() - startTime < maxWait) {
    const mailRows = await page.locator("div[role='row']").evaluateAll((rows) => {
      return rows
        .map((row) => {
          ...
          const time = titleEl
            ? new Date(titleEl.getAttribute('title') || titleEl.getAttribute('aria-label') || '').getTime()
            : 0;
    ...
    // new email invalid → refresh inbox
    await page.reload();
  ...
  if (!latestMailItem) {
    await context.close();
    throw new Error("Not found any new email with 'log In' subject");
  }
